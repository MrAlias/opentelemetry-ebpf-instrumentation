...
static __always_inline void cleanup_tcp_trace_info_if_needed_mode(pid_connection_info_t *pid_conn,
                                                                  u8 cleanup_java_remote_parent) {
    ...
    if (existing->direction == TCP_RECV) {
        cleanup_trace_info_mode(existing, pid_conn, cleanup_java_remote_parent);
    }
}

static __always_inline void cleanup_tcp_trace_info_if_needed(pid_connection_info_t *pid_conn) {
    cleanup_tcp_trace_info_if_needed_mode(pid_conn, 1);
}
