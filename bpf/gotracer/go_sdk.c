...
typedef struct go_auto_target {
    u64 generation;
    u64 process_start_ticks;
    u64 process_lifecycle_epoch;
} go_auto_target_t;
...
static __always_inline u8 go_auto_target_generation(u64 *generation) {
    ...
    go_auto_target_t *current = bpf_map_lookup_elem(&go_auto_targets, &pid);
    if (!current || !generation || !current->generation || !current->process_start_ticks ||
        !go_process_lifecycle_epoch_matches(pid, current->process_lifecycle_epoch)) {
        return 0;
    }

    u64 start_ticks = 0;
    if (!current_process_start_ticks(&start_ticks) || start_ticks != current->process_start_ticks) {
        return 0;
    }
