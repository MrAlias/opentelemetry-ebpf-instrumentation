...
commit_ssl_prewrite_transport_local(const ssl_prewrite_value_t *prewrite) {
    if (!prewrite || !prewrite->ssl || prewrite->trace.pid != prewrite->connection.pid ||
        ...
        tp_info_pid_provenance(&prewrite->trace) != k_tp_provenance_ssl_prewrite ||
...
static __noinline void bpf_sock_ops_parse_hdr_cb(struct bpf_sock_ops *skops,
    ...
    tp_info_pid_set_provenance(
        &tp, exact_flags ? k_tp_provenance_tcp_exact_flags : k_tp_provenance_tcp_legacy);
