...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `internal_metrics.prometheus.address` | `string` | `OTEL_EBPF_INTERNAL_METRICS_PROMETHEUS_ADDRESS` |  |  |  |  |
...
| `javaagent.remote_parent.transport` | `string` | `OTEL_EBPF_JAVA_REMOTE_PARENT_TRANSPORT` | `disabled` | `auto`, `disabled`, `getsockopt`, `unix` |  |  |
| `javaagent.remote_parent.ttl` | `duration` | `OTEL_EBPF_JAVA_REMOTE_PARENT_TTL` | `30s` | `30s`, `5m`, `1ms`, etc |  |  |
...
| `otel_traces_export.export_timeout` | `duration` | `OTEL_EBPF_OTLP_TRACES_EXPORT_TIMEOUT` |  | `30s`, `5m`, `1ms`, `1us`, etc |  | Bounds each OTLP trace export attempt. It uses Go duration syntax and must resolve to at least one nanosecond; explicit zero, negative, sub-nanosecond, and overflowing values fail configuration decoding or validation. The existing five-second limit applies when unset. |
