...
| `otel_traces_export.export_timeout` | *No semantics-preserving OTel declarative path* | Unsupported in the current converter: OBI renews this deadline for each Collector retry attempt, while declarative batch-processor and OTLP-exporter timeouts bound the whole export call (including exporter retries). Copying the value would silently change retry lifetime in either direction. |
