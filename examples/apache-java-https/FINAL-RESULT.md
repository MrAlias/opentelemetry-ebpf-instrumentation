...
| 6. Precedence, concurrency, keepalive, failure, and compatibility cases are exercised | fail | Retained controls pass precedence, concurrency, keepalive, primary and Unix fail-open/fault cases, [current Unix sibling/same-cgroup/stale controls](evidence/otel-unix-tls13-6c4a2505/README.md), and [full primary same/sibling/live-descriptor/stale/malformed-response controls](evidence/otel-getsockopt-tls13-e8db066a/README.md). The [retained official-agent runtime matrix](focused-validation/official-agent-runtime-9b66f39e/README.md) closes #27, #28, and #35 within their declared stock-agent scopes with checksum-verified OpenTelemetry and Splunk agent controls for Java 8/11/17/21 startup, repeated extraction, nested Jetty/Servlet instrumentation where Jetty 11 is supported, depth/cycle/late outcomes, Netty, executor, cleanup, forced carrier reuse, and Java 21 handoff. The [TLS wire-record/decrypted-callback fixture](evidence/otel-getsockopt-tls13-8282d2ed/README.md) passes on a nested Java-to-loopback-Netty connection, but its exact-parent Apache-to-Java trigger is a separate path; the same-request conjunction remains untested for #34. A Unix application-descriptor mismatch is not applicable because Unix opens a fresh broker socket and authorizes peer/TID/capability instead of accepting the application FD; its meaningful peer and endpoint analogues pass in the retained Unix cells. Permanent process-lifetime OBI absence, explicit stale-generation mismatch, genuine TID/PID reuse, an `auto` primary-unavailable-plus-fallback-unavailable application control, and current non-evicting HASH-capacity rejection/recovery remain untested for #36. The [focused diagnostic nondisclosure matrix](focused-validation/diagnostic-nondisclosure-f8775328d54a-6a2fe52aac6eab28/README.md) closes #39 within its declared scope without promoting a full acceptance or compatibility cell. Literal retained-evidence gaps for #34 and #40 are listed below. The [#37 benchmark](BENCHMARK.md) and unexecuted [#38 compatibility](COMPATIBILITY.md) rows also prevent the complete workstream from passing. The [VM-gated focused preflight](focused-validation/rhel96-kernel-sockopt-4fe50533/README.md) is a narrower retained component result, not a substitute for this full application evidence. |
...
| TLS record split/coalescing across Java callbacks | partial | [nested wire-record/decrypted-callback fixture and separate outer exact-parent graph](evidence/otel-getsockopt-tls13-8282d2ed/README.md#retained-proof); the current harness now requires an explicit same-request Apache-client/Java-server/record-callback conjunction, but no clean retained bundle has yet passed that gate |
...
| Live handoff-map pressure | untested for the current non-evicting HASH design | The retained [pressure accounting](evidence/otel-getsockopt-tls13-e8db066a/scenario-pressure-status.json) and [cleanup and recovery](evidence/otel-getsockopt-tls13-e8db066a/map-pressure-summary.json) cover the superseded LRU-eviction design; a fresh run must prove capacity rejection without evicting admitted tickets, exact accounting, cleanup, and recovery. |
...
| --- | --- | --- |
| #11 primary `getsockopt` spike | open: the accepted [transport ADR](../../devdocs/adr/001-java-remote-parent-transport.md) uses the accepted application socket for data and a dummy socket only for capability probing. The retained [primary acceptance](evidence/otel-getsockopt-tls13-e8db066a/README.md), [RHEL-kernel preflight](focused-validation/rhel96-kernel-sockopt-4fe50533/README.md), and [eight-worker packaged-JVM result](focused-validation/packaged-jvm-transports-rhel96-a86faf01/README.md) cover valid, missing, stale, deterministic ABI/error, privilege, attach, raw-JNI, and bridge/provider-JNI behavior without giving the Java child BPF capabilities, map descriptors, or bpffs access. The bounded decision is GO for continued PoC evaluation and NO-GO for production readiness. | finish #11's representative-upstream, cgroup-mode, namespace, delegated-cgroup, and nested-placement questions; report RHEL 8/backport status as unknown unless directly tested. The focused benchmark is non-acceptance evidence and does not promote #37 or #38. |
| #18 credential-checked Unix fallback | pass at the declared transport scope: [Unix-server tests](../../pkg/internal/javabridge/server_linux_test.go) cover valid/missing/stale, unauthorized one-shot handling, malformed/version-skew framing, overload, and bounded timeout; [lifecycle tests](../../pkg/internal/ebpf/tpinjector/tpinjector_test.go) cover restart; [Java selection](../../pkg/internal/java/agent/src/main/java/io/opentelemetry/obi/java/bridge/RemoteParentTransportConfiguration.java) and the [native provider](../../pkg/internal/java/agent/src/main/java/io/opentelemetry/obi/java/bridge/NativeRemoteParentProvider.java) perform one negotiated selection and permit request-triggered reprobe only after a reconfiguration-class failure and backoff. The [full forced-Unix acceptance](evidence/otel-unix-tls13-6c4a2505/README.md) retains peer/TID/capability, endpoint, permissions, fault, recovery, and exact-parent controls. The focused [packaged-JVM comparison](focused-validation/packaged-jvm-transports-rhel96-a86faf01/README.md) retains raw- and provider-JNI miss/hit/stale/timeout latency and thread-allocation samples. | none for #18's declared scope. The packaged-JVM record alone remains non-acceptance evidence; sustained application performance and broader environment/security expansion remain tracked by #37, #38, and #40 rather than reopening #18. |
...
| #20 JNI take/discard API | pass at the declared JNI scope: the accepted ADR supersedes the proposed cached dummy-socket data path with one bounded `getsockopt` call on the accepted socket. The [native tests](../../pkg/internal/java/agent/src/test/c/remote_parent_jni_test.c) and [canonical vectors](../../testdata/java-remote-parent-v1-vectors.txt) cover take/discard, every status, fixed capacity, malformed/truncated/versioned records, endianness, one-shot calls, selection, deadlines, concurrency, and cleanup. The reverse-transport [BPF object](../../bpf/javabridge/javabridge.c) does not use `bpf_probe_write_user`; unrelated instrumentation elsewhere in the repository is outside this claim. The native test target enables AddressSanitizer and UndefinedBehaviorSanitizer in [Makefile.jni](../../pkg/internal/java/agent/Makefile.jni), is wired into the [Gradle check](../../pkg/internal/java/agent/build.gradle.kts), and passed in the source-revision-matched [Java 21 CI job](https://github.com/MrAlias/opentelemetry-ebpf-instrumentation/actions/runs/31842247862/job/94901428561). The focused [packaged-JVM result](focused-validation/packaged-jvm-transports-rhel96-a86faf01/README.md) used a capability-free Java child with no BPF descriptors and retains raw- and provider-JNI hit/miss/stale/timeout latency and thread-allocation samples. | none for #20's declared JNI scope. The focused microbenchmark is not application or production evidence; sustained resource/performance and compatibility work remains under #37 and #38. |
| #23 Java-helper compatibility and failures | open: source, JNI, official-agent, executor, Netty, virtual-thread, attach/restart, and bounded failure tests are broad; the retained privileged application and packaged-JVM production-provider evidence is Java 21/`amd64` only | retain current-revision production OBI/JNI-provider executions on Java 8, 11, 17, and 21 plus native `arm64`, covering every named I/O surface, attach order, absence/failure mode, and repeated lifecycle with asserted FD, thread, direct-buffer, classloader, and request-state recovery. A #38 compatibility smoke or disabled/test provider does not satisfy this lifecycle scope. |
| #27 agent compatibility | pass: the [versioned stock-agent runtime matrix](focused-validation/official-agent-runtime-9b66f39e/README.md) retains exact successful Java 8/11/17/21 Linux `X64`/`x86_64` cells for checksum-verified, unmodified OpenTelemetry 2.28.1 and Splunk 2.28.0 agents; it verifies extension startup, Jetty 11 where supported, Netty, Java 21 concurrency, API/SPI/classloader alignment, forbidden-class absence, and deterministic boundary rejection. Existing Java 21 [OpenTelemetry](evidence/otel-getsockopt-tls13-e8db066a/README.md) and [Splunk](evidence/splunk-getsockopt-tls13-47237792/README.md) privileged Compose cells supply the real OBI attach/bridge/cleanup and exact-parentage boundaries. | none for issue #27's declared two-agent range; this does not establish `arm64`, promote Java 8/11/17 to privileged Compose passes, close issue #23, or expand issue #38 |
| #28 Java extraction matrix | applicable getsockopt and Unix application cases passed with exact graphs and one-shot counter assertions; `OfficialAgentJettyRuntimeTest` covers registered re-extraction, nested Jetty/Servlet advice, precedence, async dispatch, keepalive, and parallel connections, while the companion Netty and Java 21 probes cover inbound and executor/virtual-thread handoff with pinned, checksum-verified stock OpenTelemetry and Splunk agents | none; the [retained official-agent runtime matrix](focused-validation/official-agent-runtime-9b66f39e/README.md) passed across Java 8, 11, 17, and 21, and broader environment expansion remains tracked by #38 rather than reopening #28 |
...
| #34 stress | retained keepalive, pipelining, concurrency, churn, fd/port reuse, slow-body, timeout/retry, pressure/cleanup, reason-coded outcome, and exact-parent controls pass; the [current TLS 1.3 supplement](evidence/otel-getsockopt-tls13-8282d2ed/README.md) adds actual nested TLS-record split/coalescing with exact decrypted callbacks and ordering; the current harness additionally binds each supported Apache-to-Java request's exact parent graph to its own TLS-record/decrypt/parser evidence and preserves the unsupported two-request Go TLS receive as a reason-coded ambiguity | retain a clean full bundle that passes the new same-request conjunction and receive-map recovery gates |
| #35 handoff | retained application graphs cover blocking, executor, virtual-thread, Netty worker, redispatch, and inbound-Netty paths; stock-agent Jetty tests exercise exact parents plus depth-64, ancestor-cycle, late-framework, and transport-missing outcomes, while Netty, packaged executor, and Java 21 controls enforce relay cleanup, unwind/recovery, concurrency, migration, and carrier/worker reuse | none; the [retained official-agent runtime matrix](focused-validation/official-agent-runtime-9b66f39e/README.md) passed across Java 8, 11, 17, and 21, and broader framework/environment expansion remains tracked by #38 |
| #36 fail-open | retained primary and Unix bounded startup absence/late attach, failed attach, disabled, timeout, disconnect, overload, restart, TTL-stale, version, declared-size, truncated-frame, zero-ID, historical LRU-era map pressure, FD reuse, W3C precedence, and recovery controls pass for their transport-applicable cells; the focused RHEL kernel preflight retains the direct JVM fault component fixture | retain application-level controls for permanent process-lifetime OBI absence, an explicit stale-generation mismatch, genuine TID/PID reuse, `auto` with both primary and fallback unavailable, and current non-evicting HASH-capacity rejection plus cleanup/recovery; the VM-gated direct component fixture remains narrower than an application compatibility cell |
| #37 benchmark | predeclared matrix, repeated bounded workload, resource/map snapshots, a focused digest-pinned RHEL kernel result with all six Go transport/provider gates passed, and a [digest-pinned RHEL 9.6 packaged-JVM transport result](focused-validation/packaged-jvm-transports-rhel96-a86faf01/README.md) retaining all 14 eight-worker raw-JNI and bridge/provider-JNI `getsockopt`/Unix miss, hit, stale, and Unix-timeout series with 2,048 samples per series, paired thread-allocation observations, and all predeclared gates passed | the packaged-JVM record is focused non-acceptance evidence; retain sustained concurrent application throughput/latency, application allocation profiles, process/native resource growth, attributable map growth, CPU isolation, run-to-run variance, and the required cgroup/namespace matrix evidence |
| #38 compatibility | the bounded V3 campaign, exact 45-ID roster, provider adapters, sealer, and collector are source-built; execution remains pending. Five historical exact cells are retained on Linux `7.0.0-1009-aws` (distribution unrecorded), unified-v2, `amd64`, and Java 21: OpenTelemetry 2.28.1 on forced `getsockopt` and Unix at TLS 1.2/1.3 where linked in [COMPATIBILITY.md](COMPATIBILITY.md), plus Splunk 2.28.0 on forced `getsockopt`/TLS 1.3. They remain exact-revision observations rather than current-source or cross-product proof. | execute the source-built V3 campaign and retain explicit pass/fail/unsupported/untested records for representative kernels, host/container and cgroup topologies, native `arm64`, privileged Java 8/11/17/21, both pinned agents, both TLS versions, and both forced transports. No V3 pass is currently materialized; do not infer unexecuted Cartesian combinations. |
| #39 diagnostics | pass within #39's declared scope: the [focused eight-cell diagnostic nondisclosure matrix](focused-validation/diagnostic-nondisclosure-f8775328d54a-6a2fe52aac6eab28/README.md) retained checksum-verified OpenTelemetry 2.28.1 and Splunk 2.28.0 on source-configured Java 21/TLS 1.3 across forced `getsockopt`/Unix and INFO/DEBUG; all six governed surfaces per cell were schema-valid and complete/bounded, reconstructed canaries had zero matches, per-control journal evidence remained reason-coded, and metric labels are bounded at 792 operation tuples plus one separate fixed attach scalar | none for #39's declared scope; this focused `acceptance_evidence:false` record does not promote a clean full compatibility cell, and broader environments remain tracked by #38 |
| #40 security | retained Unix forged/flood/path/permission matrix, sibling and same-cgroup topology, exact-parent victims, stale-TTL rejection, and recovery pass; the [current primary acceptance result](evidence/otel-getsockopt-tls13-e8db066a/README.md) adds same/sibling isolated unauthorized windows, same-JVM wrong-live-socket and duplicated-descriptor controls, exact-parent victims, stale/malformed rejection, and recovery; the [focused diagnostic nondisclosure matrix](focused-validation/diagnostic-nondisclosure-f8775328d54a-6a2fe52aac6eab28/README.md) passes the runtime diagnostic-side-channel threat only | retain explicit stale-generation mismatch, genuine TID/PID reuse, primary wrong-current-TID/logical-execution, live non-evicting handoff-map capacity evidence, every other `untested` abuse row in [SECURITY.md](SECURITY.md), and broader environment coverage tracked by #38 |
