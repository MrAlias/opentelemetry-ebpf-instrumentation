...
| #37 benchmark | predeclared matrix, repeated bounded workload, resource/map snapshots, a focused digest-pinned RHEL kernel result with all six Go transport/provider gates passed, and a [local upstream-host packaged-JVM result](focused-validation/packaged-jvm-getsockopt-75aa1a06/README.md) with 256 accepted-socket miss plus 256 hit JNI/`getsockopt` samples passing the 1 ms p99 gates | the local packaged-JVM record has no public CI locator and is non-acceptance evidence; execute the packaged-JVM cell on RHEL 9 and retain sustained application, allocation, resource-growth, concurrency, and variance evidence |
