...
| #39 diagnostics | retained OpenTelemetry and Splunk per-scenario assertion statuses, current availability schema, V2 snapshots, delayed-first-export suppression evidence, and bounded public diagnostic summaries passed; the current harness durably captures the last validated fixed-schema Java bridge boundary before fault recovery, seals and embeds that boundary if recovery fails, and discards it after successful recovery, while default-level OBI availability logs expose only fixed stage/reason fields | retain paired bounded OBI metric evidence for every negative boundary and a runtime endpoint/default-log/debug-log non-disclosure audit; execute remaining agent/environment cells |
