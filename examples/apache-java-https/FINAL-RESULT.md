...
| #38 compatibility | five historical exact cells are retained on Linux `7.0.0-1009-aws` (distribution unrecorded), unified-v2, `amd64`, and Java 21: OpenTelemetry 2.28.1 on forced `getsockopt` and Unix at TLS 1.2/1.3 where linked in [COMPATIBILITY.md](COMPATIBILITY.md), plus Splunk 2.28.0 on forced `getsockopt`/TLS 1.3. They remain exact-revision observations rather than current-source or cross-product proof. | build and execute the bounded V3 compatibility campaign with explicit pass/fail/unsupported/untested records for representative kernels, host/container and cgroup topologies, native `arm64`, privileged Java 8/11/17/21, both pinned agents, both TLS versions, and both forced transports. Retain a durable per-cell/aggregate schema; do not infer unexecuted Cartesian combinations. |
