...
| stale entry past TTL | miss; never a parent | untested | untested | the unexecuted `primary-w3c-stale` source control forces a `1ns` primary retrieval TTL while retaining prewrite and cleanup, then verifies W3C precedence plus recovery; retain clean full transport-specific evidence |
