...
The retained bundles are immutable evidence for their recorded revisions. Both
currently predate the `availability` rename, so their OBI metric deltas can
contain `operation="select"`. In those historical revisions, `select` means
only OBI-side transport readiness or preference; it is neither Java helper
selection nor proof that a request used that transport. The current schema uses
`operation="availability"` and has an eleven-operation, 792-series upper bound,
as documented in the [Java remote-parent bridge
guide](../../../devdocs/java-remote-parent-bridge.md). Checksum verification
authenticates the retained artifacts; it does not recast their historical
schema.
