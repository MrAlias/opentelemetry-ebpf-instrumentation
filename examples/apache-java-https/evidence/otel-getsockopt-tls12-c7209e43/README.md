# OpenTelemetry getsockopt TLS 1.2 acceptance

Result: **pass**

This bounded, reviewer-facing subset of a clean full run is published under
the non-runtime-derived evidence ID **otel-getsockopt-tls12-c7209e43**. It
...
| Field | Value |
| --- | --- |
| Public evidence ID | otel-getsockopt-tls12-c7209e43 |
| Source revision | c7209e4306694ed2f6fe4d2bb813d7b632915dd2 |
| Source state | clean |
| Source-tree manifest SHA-256 | 38cfb80ddbb8762be69531b2ca93d69e43d6d72001ab2fca50c8fe33da24918d |
| Invocation | examples/apache-java-https/run.sh --transport getsockopt --agent otel --tls TLSv1.2 --command-timeout 1800 |
...
| Apache-to-inbound-Netty HTTPS fixture, HTTP/1.1, TLS 1.2, and exact remote parent | [Netty-server graph](scenario-netty-server.json), [scenario status](scenario-netty-server-status.json), and [bounded phase deltas](phases/netty-server-after/) |
...
From this directory:
    ...
    test "$(sha256sum source-tree.manifest | cut -d' ' -f1)" = \
      38cfb80ddbb8762be69531b2ca93d69e43d6d72001ab2fca50c8fe33da24918d
    ...
    jq -e '
      ...
      .request_count == 4 and
      (.cases | length == 4) and
      all(.cases[]; .response.backend_kind == "netty" and .response.tls_protocol == "TLSv1.2")
