...
the non-runtime-derived evidence ID **otel-getsockopt-tls13-94221a91**. It
proves only the matrix cell named below. It does not turn unexecuted transports,
TLS versions, agents, kernels, cgroup topologies, architectures, or JVMs into
passing cells.
...
| Field | Value |
| --- | --- |
| Public evidence ID | otel-getsockopt-tls13-94221a91 |
| Source revision | 94221a9127553adb233a7560baa6f2a327c558b8 |
| Source state | clean |
| Source-tree manifest SHA-256 | 9f59e7714add70411e9669e891bf3105d31799fa26b994c2a2ecff87efa42947 |
| Invocation | ./examples/apache-java-https/run.sh --transport getsockopt --agent otel --tls TLSv1.3 --command-timeout 1800 --readiness-timeout 120 |
...
| Cgroup | unified v2 |
| Runner result | 33 passed, 1 expected unsupported, 0 failed |
...
native attempt. It does not prove that a particular request used that transport;
the exact trace assertions in the scenario graphs provide that independent
request-level proof.
...
| Low-cardinality diagnostics and duplicate suppression | phases Java and OBI deltas, [allowlisted suppression metric](duplicate-suppression.json), and every scenario status |
...
retained request-header attribute is the bounded synthetic x-obi-demo-id marker
used to join each test request to its trace graph.

The full runtime bundle contained bulk metric snapshots and operational logs.
They are intentionally omitted. Retained OBI metric files contain only bridge
operation/status/transport labels and per-phase deltas; absolute before/after
values and unrelated host metrics are excluded. The retained Java diagnostic
deltas and scenario results are sufficient to reproduce the acceptance
decisions. The original source manifest is retained so its digest can be
checked independently. [SANITIZATION.md](SANITIZATION.md) enumerates the
transformed and omitted operational records. The checksum manifest
authenticates the retained public subset, not the omitted raw bundle.
...
From this directory:
    ...
    test "$(sha256sum source-tree.manifest | cut -d' ' -f1)" = \
      9f59e7714add70411e9669e891bf3105d31799fa26b994c2a2ecff87efa42947
