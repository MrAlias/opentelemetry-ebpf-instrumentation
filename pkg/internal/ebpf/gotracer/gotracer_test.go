...
type lifecycleEpochPut struct {
	pid   uint32
	epoch uint64
}

type recordingLifecycleEpochMap struct {
	mu      sync.Mutex
	next    uint64
	values  map[uint32]uint64
	puts    []lifecycleEpochPut
	deletes []uint32
}

func (m *recordingLifecycleEpochMap) newEpoch() (uint64, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.next++
	if m.next == 0 {
...
func (m *recordingLifecycleEpochMap) lookup(pid uint32) (uint64, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	epoch, ok := m.values[pid]
	if !ok {
...
func (m *recordingLifecycleEpochMap) put(pid uint32, epoch uint64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.values[pid] = epoch
	m.puts = append(m.puts, lifecycleEpochPut{pid: pid, epoch: epoch})
	return nil
}

func (m *recordingLifecycleEpochMap) delete(pid uint32) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	delete(m.values, pid)
	m.deletes = append(m.deletes, pid)
	return nil
}

func installLifecycleEpochTestSeams(tracer *Tracer) *recordingLifecycleEpochMap {
	epochs := &recordingLifecycleEpochMap{
		next:   1000,
		values: map[uint32]uint64{},
	}
	tracer.validateRuntimeMetricOwnerForTest = func(app.PID, *exec.FileInfo) error { return nil }
	tracer.newGoProcessLifecycleEpochForTest = epochs.newEpoch
	tracer.lookupGoProcessLifecycleEpochForTest = epochs.lookup
	tracer.putGoProcessLifecycleEpochForTest = epochs.put
	tracer.deleteGoProcessLifecycleEpochForTest = epochs.delete
	return epochs
}

func testGoProcessOwner(pid app.PID, dev, ino, start uint64) *exec.FileInfo {
	return exec.New(exec.Init{
		Pid:               pid,
		Dev:               dev,
		Ino:               ino,
		ProcessStart:      start,
		ProcessInstanceID: start + uint64(pid) + 1,
	})
}
...
func (f *allowRecordingFilter) AllowPID(
	...
	_ *exec.FileInfo,
	_ *exec.FileInfo,
	_ ebpfcommon.PIDType,
	commit ...ebpfcommon.PIDAdmissionCommit,
) bool {
	f.allowed++
	if !f.admitted {
		return false
...
func TestRuntimeMetricTargetReplacementRetiresPredecessorBeforeEarlyReturn(t *testing.T) {
	...
	tracer.registerRuntimeMetricTarget(key.pid, key.ns, replacement, replacement, 1)
...
func TestRuntimeMetricTargetReplacementRetiresPredecessorAcrossNamespaces(t *testing.T) {
	...
	predecessor := testGoProcessOwner(pid, 5, 10, 100)
	replacement := testGoProcessOwner(pid, 6, 11, 200)
	var deleted []BpfPidInfo
	registered := 0
	tracer := &Tracer{
	...
	tracer.registerRuntimeMetricTarget(pid, 8, replacement, replacement, 1)
...
func TestRuntimeMetricTargetCleanupFailureBlocksReplacementAdmission(t *testing.T) {
	...
	predecessor := testGoProcessOwner(pid, 5, 10, 100)
	replacement := testGoProcessOwner(pid, 6, 11, 200)
	filter := &allowRecordingFilter{admitted: true}
	publicationCalls := 0
	tracer := &Tracer{
		log:                             slog.New(slog.NewTextHandler(io.Discard, nil)),
		pidsFilter:                      filter,
		...
		putRuntimeMetricTargetForTest: func(_ BpfPidInfo, value BpfGoRuntimeMetricTargetT) error {
			require.Empty(t, value)
			return errors.New("injected BPF zero fallback failure")
		},
		registerRuntimeMetricTargetForTest: func(
			app.PID, uint32, *exec.FileInfo, *exec.FileInfo,
		) {
	...
	installLifecycleEpochTestSeams(tracer)

	admitted := tracer.AllowPID(pid, 8, replacement, replacement)

	assert.False(t, admitted)
	assert.Equal(t, 1, filter.allowed, "replacement admission must be attempted transactionally")
...
func TestRuntimeMetricTargetDeleteFailureZeroesAndRetiresReplacement(t *testing.T) {
	const pid = app.PID(42)
	oldKey := runtimeMetricTargetKey{pid: pid, ns: 7}
	oldPIDInfo := BpfPidInfo{HostPid: uint32(pid), UserPid: 3, Ns: oldKey.ns}
	predecessor := testGoProcessOwner(pid, 5, 10, 100)
	...
	tracer := &Tracer{
		...
		putRuntimeMetricTargetForTest: func(pidInfo BpfPidInfo, value BpfGoRuntimeMetricTargetT) error {
			zeroPuts++
			assert.Equal(t, oldPIDInfo, pidInfo)
	...
	assert.Equal(t, 1, zeroPuts)
	assert.NotContains(t, tracer.runtimeMetricTargetKeys, oldKey)
	assert.NotContains(t, tracer.runtimeMetricTargetOwners, oldKey)
}

func TestRuntimeMetricTargetPartialCleanupFailureKeepsOnlyUnneutralizedTarget(t *testing.T) {
	const pid = app.PID(42)
	firstKey := runtimeMetricTargetKey{pid: pid, ns: 7}
	secondKey := runtimeMetricTargetKey{pid: pid, ns: 8}
	firstPIDInfo := BpfPidInfo{HostPid: uint32(pid), UserPid: 3, Ns: firstKey.ns}
	secondPIDInfo := BpfPidInfo{HostPid: uint32(pid), UserPid: 4, Ns: secondKey.ns}
	firstValue := BpfGoRuntimeMetricTargetT{
		Generation: 11, MemstatsAddr: 0x1111, ProcessStartTicks: 100,
	}
	secondValue := BpfGoRuntimeMetricTargetT{
		Generation: 22, MemstatsAddr: 0x2222, ProcessStartTicks: 101,
	}
	firstOwner := testGoProcessOwner(pid, 5, 10, 100)
	secondOwner := testGoProcessOwner(pid, 5, 10, 101)
	replacement := testGoProcessOwner(pid, 6, 11, 102)
	targets := map[BpfPidInfo]BpfGoRuntimeMetricTargetT{
		firstPIDInfo:  firstValue,
		secondPIDInfo: secondValue,
	}
	var deletes []BpfPidInfo
	var zeroPuts []BpfPidInfo
	filter := &allowRecordingFilter{admitted: true}
	tracer := &Tracer{
		log:        slog.New(slog.NewTextHandler(io.Discard, nil)),
		pidsFilter: filter,
		runtimeMetricTargetKeys: map[runtimeMetricTargetKey]BpfPidInfo{
			firstKey: firstPIDInfo, secondKey: secondPIDInfo,
		},
		runtimeMetricTargetOwners: map[runtimeMetricTargetKey]*exec.FileInfo{
			firstKey: firstOwner, secondKey: secondOwner,
		},
		deleteRuntimeMetricTargetForTest: func(pidInfo BpfPidInfo) error {
			deletes = append(deletes, pidInfo)
			if len(deletes) == 2 {
				return errors.New("injected second delete failure")
			}
			delete(targets, pidInfo)
			return nil
		},
		putRuntimeMetricTargetForTest: func(
			pidInfo BpfPidInfo, value BpfGoRuntimeMetricTargetT,
		) error {
			zeroPuts = append(zeroPuts, pidInfo)
			require.Empty(t, value)
			return errors.New("injected zero fallback failure")
		},
	}
	installLifecycleEpochTestSeams(tracer)

	admitted := tracer.AllowPID(pid, 9, replacement, replacement)

	assert.False(t, admitted)
	assert.Equal(t, 1, filter.allowed)
	assert.Equal(t, []BpfPidInfo{firstPIDInfo, secondPIDInfo}, deletes)
	assert.Equal(t, []BpfPidInfo{secondPIDInfo}, zeroPuts)
	assert.Equal(t, map[BpfPidInfo]BpfGoRuntimeMetricTargetT{secondPIDInfo: secondValue}, targets)
	assert.Equal(t, map[runtimeMetricTargetKey]BpfPidInfo{
		secondKey: secondPIDInfo,
	}, tracer.runtimeMetricTargetKeys)
	assert.Equal(t, map[runtimeMetricTargetKey]*exec.FileInfo{
		secondKey: secondOwner,
	}, tracer.runtimeMetricTargetOwners)
}

func TestAllowPIDRejectedExactOwnerSkipsGoProcessPublication(t *testing.T) {
	const pid = app.PID(42)
	const ns = uint32(7)
	filter := &allowRecordingFilter{admitted: false}
	predecessor := testGoProcessOwner(pid, 5, 10, 100)
	const predecessorEpoch = uint64(77)
	runtimeKey := runtimeMetricTargetKey{pid: pid, ns: ns}
	runtimePIDInfo := BpfPidInfo{HostPid: uint32(pid), UserPid: uint32(pid), Ns: ns}
	runtimeValue := BpfGoRuntimeMetricTargetT{Generation: 17, MemstatsAddr: 0x1234}
	runtimeTargets := map[BpfPidInfo]BpfGoRuntimeMetricTargetT{runtimePIDInfo: runtimeValue}
	targets := newRecordingTargetMap()
	targets.entries[uint32(pid)] = BpfGoAutoTargetT{
		Generation:            9,
		ProcessStartTicks:     predecessor.ProcessStartTime(),
		ProcessLifecycleEpoch: predecessorEpoch,
	}
	attempts := &recordingMapKeyDeleter{errors: map[uint8]error{}}
	activationLink := &activationCountingCloser{}
	existingState := goAutoSDKTargetState{
		generation:     9,
		owner:          predecessor,
		dev:            5,
		ino:            10,
		startTime:      100,
		lifecycleEpoch: predecessorEpoch,
	}
	identityReads := 0
	attachCalls := 0
	...
	tracer := &Tracer{
		...
		pidsFilter: filter,
		runtimeMetricTargetKeys: map[runtimeMetricTargetKey]BpfPidInfo{
			runtimeKey: runtimePIDInfo,
		},
		runtimeMetricTargetOwners: map[runtimeMetricTargetKey]*exec.FileInfo{
			runtimeKey: predecessor,
		},
		goAutoSDKTargets: map[app.PID]goAutoSDKTargetState{pid: existingState},
		goProcessLifecycleEpochs: map[app.PID]goProcessLifecycleEpochState{
			pid: {owner: predecessor, epoch: predecessorEpoch},
		},
		...
		goAutoSDKActivationLinks: map[goAutoSDKActivationLinkKey]goAutoSDKActivationLink{
			{pid: pid, generation: existingState.generation}: {
				link: &onceCloser{closer: activationLink},
			},
		},
		goAutoSDKTargetMap:  targets,
		goAutoSDKAttemptMap: attempts,
		goAutoSDKProcessStartTime: func(app.PID) (uint64, error) {
			identityReads++
			return 200, nil
		},
		attachGoAutoSDKProbe: func(
			goAutoSDKActivationProbe, app.PID, uint64, uint64, uint64,
		) (io.Closer, error) {
			attachCalls++
		...
		deleteRuntimeMetricTargetForTest: func(pidInfo BpfPidInfo) error {
			delete(runtimeTargets, pidInfo)
			return nil
		},
	}
	replacement := testGoProcessOwner(pid, 6, 11, 200)

	// This models PIDsFilter restoring an incumbent after its post-publication
	// exact-owner validation rejects the replacement. Numeric ValidPID remains
	// true through IdentityPidsFilter, but the replacement itself was not admitted.
	require.True(t, filter.ValidPID(pid, ns, ebpfcommon.PIDTypeGo))
	tracer.AllowPID(pid, ns, replacement, replacement)

	assert.Equal(t, 1, filter.allowed)
	assert.Zero(t, identityReads)
	assert.Zero(t, attachCalls)
	assert.Zero(t, runtimePublications)
	assert.Equal(t, runtimePIDInfo, tracer.runtimeMetricTargetKeys[runtimeKey])
	assert.Same(t, predecessor, tracer.runtimeMetricTargetOwners[runtimeKey])
	assert.Equal(t, runtimeValue, runtimeTargets[runtimePIDInfo],
		"post-publication owner rejection must leave the incumbent BPF target intact")
	assert.Empty(t, targets.puts)
	assert.Empty(t, targets.deletes)
	assert.Equal(t, BpfGoAutoTargetT{
		Generation:            9,
		ProcessStartTicks:     predecessor.ProcessStartTime(),
		ProcessLifecycleEpoch: predecessorEpoch,
	}, targets.entries[uint32(pid)])
	assert.Empty(t, attempts.keys)
	assert.Equal(t, existingState, tracer.goAutoSDKTargets[pid])
	assert.Equal(t, goProcessLifecycleEpochState{
		owner: predecessor,
		epoch: predecessorEpoch,
	}, tracer.goProcessLifecycleEpochs[pid])
	assert.Len(t, tracer.goAutoSDKActivationLinks, 1)
	assert.Zero(t, activationLink.closes.Load())
}
...
func TestRegisterOffsetsCannotPublishRuntimeTargetBeforeAllowPID(t *testing.T) {
	...
	installLifecycleEpochTestSeams(tracer)
	fileInfo := testGoProcessOwner(42, 5, 7, 100)
...
func TestRegisterOffsetsFailsClosedWhenOffsetMapWriteFails(t *testing.T) {
	...
	assert.Empty(t, tracer.goChannelOffsetsByExecutable)
	assert.Empty(t, tracer.goRuntimeMetricMaskByExecutable)
	assert.Zero(t, tracer.currentBinary)
}
...
func TestRegisterOffsetsSeparatesSameInodeAcrossDevices(t *testing.T) {
	...
	var keys []executableIdentity
	tracer := &Tracer{
		putGoOffsetsForTest: func(key executableIdentity, _ BpfOffTableT) error {
	...
	require.Equal(t, []executableIdentity{
		goOffsetsMapKey(withoutChannelOffsets),
		goOffsetsMapKey(withChannelOffsets),
	}, keys)
	withoutIdentity := goOffsetsMapKey(withoutChannelOffsets)
	withIdentity := goOffsetsMapKey(withChannelOffsets)
	assert.False(t, tracer.goChannelOffsetsByExecutable[withoutIdentity])
	assert.True(t, tracer.goChannelOffsetsByExecutable[withIdentity])
	assert.Equal(t, goRuntimeMetricProcessorLimitMask,
		tracer.goRuntimeMetricMaskByExecutable[withoutIdentity])
	assert.True(t, hasBaseGoRuntimeMetrics(
		tracer.goRuntimeMetricMaskByExecutable[withIdentity]))
}

func TestGoOffsetsMapKeyABI(t *testing.T) {
	spec, err := LoadBpf()
	require.NoError(t, err)
	mapSpec := spec.Maps["go_offsets_map"]
	require.NotNil(t, mapSpec)

	var key executableIdentity
...
func TestGoRuntimeMetricTargetABIAppendsGoroutineMetadata(t *testing.T) {
	...
	assert.Equal(t, uintptr(120), unsafe.Sizeof(target))
...
func TestGoRuntimeMetricTargetABIAppendsGCGoalCache(t *testing.T) {
	...
	assert.Equal(t, uintptr(104), unsafe.Offsetof(target.ProcessStartTicks))
	assert.Equal(t, uintptr(112), unsafe.Offsetof(target.ProcessLifecycleEpoch))
}

func TestGoAutoSDKTargetABIIncludesExactLifecycleIdentity(t *testing.T) {
	var target BpfGoAutoTargetT

	assert.Equal(t, uintptr(24), unsafe.Sizeof(target))
	assert.Equal(t, uintptr(0), unsafe.Offsetof(target.Generation))
	assert.Equal(t, uintptr(8), unsafe.Offsetof(target.ProcessStartTicks))
	assert.Equal(t, uintptr(16), unsafe.Offsetof(target.ProcessLifecycleEpoch))
}

func TestGoProcessLifecycleTracepointsAreRequired(t *testing.T) {
	execProgram := &ebpf.Program{}
	exitProgram := &ebpf.Program{}
	tracer := &Tracer{bpfObjects: BpfObjects{BpfPrograms: BpfPrograms{
		ObiGoProcessExec: execProgram,
		ObiGoProcessExit: exitProgram,
	}}}

	tracepoints := tracer.Tracepoints()
	require.Len(t, tracepoints, 2)
	execProbe := tracepoints["sched/sched_process_exec"]
	exitProbe := tracepoints["sched/sched_process_exit"]
	assert.Same(t, execProgram, execProbe.Start)
	assert.True(t, execProbe.Required)
	assert.Same(t, exitProgram, exitProbe.Start)
	assert.True(t, exitProbe.Required)
}
...
func TestProcessBinarySelectsRecordedChannelOffsetState(t *testing.T) {
	...
	tracer := &Tracer{
		goChannelOffsetsByExecutable: map[executableIdentity]bool{
			goOffsetsMapKey(exec.New(exec.Init{Dev: withOffsets.Dev, Ino: ino})):    true,
			goOffsetsMapKey(exec.New(exec.Init{Dev: withoutOffsets.Dev, Ino: ino})): false,
		},
	}
...
func TestDuplicateAllowPIDKeepsOneActivationLink(t *testing.T) {
	...
	fileInfo := testGoProcessOwner(123, 5, 10, 100)

	tracer.AllowPID(123, 1, fileInfo, fileInfo)
	...
	assert.NotZero(t, tracer.goAutoSDKTargets[123].lifecycleEpoch)
	assert.Equal(t, tracer.goAutoSDKTargets[123].lifecycleEpoch,
		tracer.goAutoSDKTargetMap.(*recordingTargetMap).entries[123].ProcessLifecycleEpoch)
...
func TestAllowPIDRetriesProcessIdentityBeforeAttaching(t *testing.T) {
	...
	tracer.AllowPID(123, 1, fileInfo, fileInfo)
	assert.Empty(t, tracer.goAutoSDKTargets)
	assert.Empty(t, tracer.goAutoSDKActivationLinks)

	tracer.AllowPID(123, 1, fileInfo, fileInfo)

	assert.NotZero(t, tracer.goAutoSDKTargets[123].generation)
	assert.Equal(t, 1, attachCalls)
	assert.Len(t, tracer.goAutoSDKActivationLinks, 1)
}

func TestAllowPIDIdentityFailureRetiresSameFilePreviousOwner(t *testing.T) {
	identityReads := 0
	tracer := activationLifecycleTestTracer(func(app.PID) (uint64, error) {
		identityReads++
		if identityReads == 1 {
		...
		return 0, errors.New("replacement process stat unavailable")
	})
	...
	oldOwner := testGoProcessOwner(123, 5, 10, 100)
	replacement := testGoProcessOwner(123, 5, 10, 200)

	require.True(t, tracer.AllowPID(123, 1, oldOwner, oldOwner))
	require.Same(t, oldOwner, tracer.goAutoSDKTargets[123].owner)
	require.Len(t, tracer.goAutoSDKActivationLinks, 1)
	assert.Equal(t, uint64(100),
		tracer.goAutoSDKTargetMap.(*recordingTargetMap).entries[123].ProcessStartTicks)
	require.True(t, tracer.AllowPID(123, 1, replacement, replacement))

	assert.Equal(t, 1, attachCalls)
	assert.Equal(t, int32(1), oldLink.closes.Load())
	assert.Empty(t, tracer.goAutoSDKTargets)
	assert.Empty(t, tracer.goAutoSDKActivationLinks)
	assert.Empty(t, tracer.goAutoSDKTargetMap.(*recordingTargetMap).entries,
		"the predecessor generation must not stay enabled for the replacement")
}

func TestAllowPIDActivationLinkFailureDisablesPublishedGeneration(t *testing.T) {
	tracer := activationLifecycleTestTracer(func(app.PID) (uint64, error) {
		return 100, nil
	})
	tracer.attachGoAutoSDKProbe = func(
		...
		return nil, errors.New("injected process-link failure")
	}
	owner := testGoProcessOwner(123, 5, 10, 100)

	require.True(t, tracer.AllowPID(123, 1, owner, owner))
...
func TestGoAutoSDKActivationHandlesPIDReuse(t *testing.T) {
	...
	oldOwner := testGoProcessOwner(123, 5, 10, 100)
	newOwner := testGoProcessOwner(123, 5, 10, 200)

	tracer.AllowPID(123, 1, oldOwner, oldOwner)
	oldGeneration := tracer.goAutoSDKTargets[123].generation
	startTime = 200
	tracer.AllowPID(123, 1, newOwner, newOwner)
...
func TestGoAutoSDKActivationSupportsProcessesSharingInode(t *testing.T) {
	...
	firstOwner := testGoProcessOwner(123, 5, 10, 123)
	secondOwner := testGoProcessOwner(456, 5, 10, 456)
	tracer.AllowPID(123, 1, firstOwner, firstOwner)
	tracer.AllowPID(456, 1, secondOwner, secondOwner)
...
func TestGoAutoSDKActivationCleanupRaceClosesLinkOnce(t *testing.T) {
	for range 25 {
		...
		go func() {
			defer wg.Done()
			tracer.BlockPID(123, 1, fileInfo, fileInfo)
		}()
		wg.Wait()
...
func TestBlockPIDClosesActivationLinkBeforePIDReuse(t *testing.T) {
	...
	epochs := installLifecycleEpochTestSeams(tracer)
	...
	fileInfo := testGoProcessOwner(123, 5, 10, 123)

	tracer.AllowPID(123, 1, fileInfo, fileInfo)
	firstGeneration := tracer.goAutoSDKTargets[123].generation
	tracer.BlockPID(123, 1, fileInfo, fileInfo)
	assert.NotContains(t, epochs.values, uint32(123),
		"retired lifecycle keys must not evict idle live processes from the bounded LRU")
	assert.Equal(t, []uint32{123}, epochs.deletes)
	assert.NotContains(t, tracer.goProcessLifecycleEpochs, app.PID(123))
	tracer.AllowPID(123, 1, fileInfo, fileInfo)
...
func TestGoAutoSDKTargetDeleteFailureFallsBackToZero(t *testing.T) {
	targets := newRecordingTargetMap()
	targets.entries[123] = BpfGoAutoTargetT{Generation: 7}
...
func TestGoAutoSDKTargetRecoveryPublishesBeforeRetiredAttemptCleanup(t *testing.T) {
	...
	assert.Equal(t, uint64(8), targets.entries[123].Generation)
...
type targetMapPut struct {
	key   uint32
	value BpfGoAutoTargetT
}

type recordingTargetMap struct {
	entries      map[uint32]BpfGoAutoTargetT
	puts         []targetMapPut
	deletes      []uint32
	putErrors    []error
	deleteErrors []error
...
func newRecordingTargetMap() *recordingTargetMap {
	return &recordingTargetMap{entries: map[uint32]BpfGoAutoTargetT{}}
}

func (m *recordingTargetMap) Put(key, value any) error {
	targetKey := *key.(*uint32)
	targetValue := *value.(*BpfGoAutoTargetT)
