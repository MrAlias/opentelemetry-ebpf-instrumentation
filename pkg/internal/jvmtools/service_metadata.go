...
const (
	...
	// ServiceVersionAttribute is the resource attribute populated from the
	// launch JAR manifest when the service has not supplied one explicitly.
	ServiceVersionAttribute  = attr.Name("service.version")
	serviceVersion           = ServiceVersionAttribute
...
func ResolveServiceMetadata(fileInfo *exec.FileInfo) error {
	...
	metadata, err := ReadServiceMetadata(fileInfo.Pid(), service)
	if err != nil {
		return err
	}
	receipt := fileInfo.BeginServiceMetadataAdmission(
		metadata.Name,
...
// ReadServiceMetadata derives only the missing Java service fields and does
// not mutate service. Keeping process inspection separate from publication
// lets discovery validate an exact process lifetime around all numeric /proc
// reads and publish the result transactionally only after those reads finish.
func ReadServiceMetadata(pid app.PID, service svc.Attrs) (ServiceMetadata, error) {
	...
	if resolutionErr != nil {
		return ServiceMetadata{}, resolutionErr
	}
	...
	return ServiceMetadata{Name: name, Version: manifest.Version}, nil
}
