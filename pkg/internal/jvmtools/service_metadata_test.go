...
func TestReadServiceMetadataIsPure(t *testing.T) {
	root := t.TempDir()
	archivePath := filepath.Join(root, "app", "orders.jar")
	writeArchive(t, archivePath, map[string]string{
		"META-INF/MANIFEST.MF": "Implementation-Title: Orders Service\r\nImplementation-Version: 2.0\r\n\r\n",
	...
	fileInfo := mockJavaProcess(t, root, []string{"-jar", "/app/orders.jar"}, nil)
	before := fileInfo.ServiceAttrs()

	metadata, err := ReadServiceMetadata(fileInfo.Pid(), before)

	require.NoError(t, err)
	assert.Equal(t, ServiceMetadata{Name: "Orders Service", Version: "2.0"}, metadata)
	assert.Equal(t, before, fileInfo.ServiceAttrs(),
		"process inspection must not publish service metadata")
	assert.False(t, fileInfo.AutoName())
}
