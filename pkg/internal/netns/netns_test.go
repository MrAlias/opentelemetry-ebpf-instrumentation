...
func TestWithNetNSFDPinsNamespaceAndKeepsCallerDescriptorOpen(t *testing.T) {
	target, err := os.Open(selfNetNS)
	require.NoError(t, err)
	defer target.Close()

	outer := threadNetNS(t)
	var observed string
	require.NoError(t, WithNetNSFD(int(target.Fd()), func() error {
		var readErr error
		observed, readErr = os.Readlink(selfNetNS)
		return readErr
	}))
	assert.Equal(t, outer, observed)

	var stat unix.Stat_t
	require.NoError(t, unix.Fstat(int(target.Fd()), &stat),
		"WithNetNSFD must close only its independent duplicate")
}

func TestWithNetNSFDRejectsInvalidInputs(t *testing.T) {
	assert.Error(t, WithNetNSFD(-1, func() error { return nil }))
	target, err := os.Open(selfNetNS)
	require.NoError(t, err)
	defer target.Close()
	assert.Error(t, WithNetNSFD(int(target.Fd()), nil))
}
