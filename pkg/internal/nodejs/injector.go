...
func (i *NodeInjector) injectViaConn(conn net.Conn) error {
	return i.injectViaConnValidated(conn, nil)
}

func (i *NodeInjector) injectViaConnValidated(
	conn net.Conn,
	beforeEvaluate func(net.Conn) error,
) error {
	...
	if beforeEvaluate != nil {
		if err := beforeEvaluate(conn); err != nil {
			return fmt.Errorf("validate exact inspector socket before Runtime.evaluate: %w", err)
		}
	}
